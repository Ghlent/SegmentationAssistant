...
### The template
The users can create plugins with SegmentationAssistant by modifying a template written in Java. No change is necessary for the program to run, but users are expected to define in code:
- The name of the plugin
- Whether they are using the point selection tool or not
- The measured variables that will appear in the results table
- The analysis that will be performed to measure those variables
- The settings used in the analysis
For more complex needs, the framework and template contemplate functions that are performed automatically every time the settings or the image change, and a section for the analysis’ preview. If that is still insufficient for the demands of the problem, all functions and classes in the framework are modifiable from the template. This all means that users need basic coding knowledge to make use of the framework, but everyone can benefit from the plugins created with it.

## Installation instructions
- For all plugins to work, you need to add segmentationFramework-x-x-x.jar to plugins/jars/ inside Fiji.app.
- Later plugins can directly be added to the plugins folder when compiled as .jar file, or drag-and-drop the .java file into the program to open the editor, where there is a run option.
- This editor is also the way users can thinker with the template of their choice (normally the Basic_Template.java).
